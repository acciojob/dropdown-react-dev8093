...
export function getCities(state){
    let currentStateData = states.find(value => value.name == state)

   return currentStateData.city.map(value => value.name)
}


export function getLandmarks(state, city){
    let currentStateData = states.find(value => value.name == state)
    let currentCityData = currentStateData.city.find(value => value.name == city)
    return currentCityData.landmarks.map(value => value.name)
}



console.log(getAllStates())
console.log(getCities("Jharkhand"))
console.log(getLandmarks("Jharkhand","Dhanbad"))